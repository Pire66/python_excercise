...
CREATE INDEX idx1 ON Planet(name);
SELECT COUNT(1)
FROM Booking B JOIN Flight F ON F.id = B.flight_id JOIN Spacecraft S ON F.spacecraft_id = S.id
WHERE (F.planet_id=(SELECT id FROM Planet WHERE name='Tiraor')) AND S.class=1 AND S.capacity<19    ;
