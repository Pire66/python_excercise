...
/*WITH T1 AS (SELECT COUNT(id) AS COUNT,UNNEST(akeys(wiki)) AS tag FROM planet WHERE wiki -> 'danger' = '1' GROUP BY tag )
		SELECT tag, COUNT FROM T1 WHERE tag IN ('dinosaurs', 'weapons', 'pirates', 'earthquakes', 'crime', 'mosquitoes') ORDER BY COUNT DESC*/

WITH T1 AS
	(SELECT  id,name,   jsonb_array_elements((wiki_jsonb ->> 'log')::jsonb) AS AR   FROM planet WHERE (((wiki_jsonb ->> 'features')::jsonb)->>'dinosaurs') = '1'),
	T2 AS (SELECT  (AR->> 'commander') AS commander_name, json_agg (CONCAT('{"planet":"', name, '", "with": "dino"}')::jsonb) AS self FROM T1 GROUP BY (AR->> 'commander'))
	SELECT CA.commander_id AS commander_id, 
		 jsonb_set(CA.data,'{selfies}', ((CA.data->>'selfies')::jsonb)|| T2.self::jsonb) AS data
		FROM CommanderAchievements  AS CA 
		JOIN commander AS Co ON CA.commander_id=Co.id 
		JOIN T2 ON T2.commander_name = Co.name 
